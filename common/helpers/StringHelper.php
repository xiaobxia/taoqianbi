...
//         $bank_name = $bank['name'];
     ...
     * 加密
     */
    public static function auto_encrypt($data, $key = 'kdkjkdkj')
    {
        $key = md5($key);
        $x = 0;
        $len = strlen($data);
     ...
     * 解密
     */
    public static function auto_decrypt($data, $key = 'kdkjkdkj')
    {
        $key = md5($key);
        $x = 0;
    ...
    public static function Jaro($string1, $string2)
    {
        ...
        $distance = (int)floor(max($str1_len, $str2_len) / 2.0);
        ...
        for ($i = 0; $i < $upperBound; $i++) {
            if (mb_substr($commons1, $i, 1) != mb_substr($commons2, $i, 1)) $transpositions++;
        }
        $transpositions /= 2.0;
    ...
    {
        $string1_len = mb_strlen($string1);
        $string2_len = mb_strlen($string2);
        $commonCharacters = '';
        $matching = 0;
        ...
        for ($i = 0; $i < $string1_len; $i++) {
            ...
            for ($j = 0; $noMatch && $j < $string2_len; $j++) {
                if ((mb_substr($string2, $j, 1) == mb_substr($string1, $i, 1)) && (abs($j - $i) <= $allowedDistance)) {
                    $noMatch = false;
                    $matching++;
                    $commonCharacters .= mb_substr($string1, $i, 1);
                }
            }
        }
