...
class TimeHelper
{
    ...
    */
    ...
    public static function DiffHours($dateA, $dateB) {
        $hours = 0;
        $hours =  intval( (( strtotime($dateA) - strtotime($dateB) ) % self::DAY ) );
        if($hours > 0) {
            $hours = intval( $hours / self::HOUR);
        }

        return $hours;
    }

    public static function DiffSenconds($dateA, $dateB) {
        $senconds = 0;
        $hours =  intval( (( strtotime($dateA) - strtotime($dateB) ) % self::DAY ) );
        if($hours > 0) {
            $senconds = intval( $hours % self::HOUR);
            if( $senconds > 0) {
                $senconds = intval( $senconds / self::SENCOND);
            }
        }
     ...
     * 返回时间差值得时分秒
     */
    public static function getHourMinSec($diff_time){
        $hour = floor($diff_time / 3600);
        $min  = floor(($diff_time - $hour * 3600) / 60);
        $sec  = floor(($diff_time - $hour * 3600 - $min * 60));
