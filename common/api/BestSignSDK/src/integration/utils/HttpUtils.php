...
class HttpUtils
{
    ...
    private function _request($url, $method = "GET", $post_data = null, $post_files = null, array $headers = array(), $auto_redirect = true, $cookie_file = null)
    {
        ...
        // set headers
        Logger::addToLog("REQUEST HEADERS: ");
        if (!is_array($headers))
        {
            $headers = array();
        }
        ...
        Logger::addToLog("RESPONSE CODE: {$http_code}");
        if ($errno != 0) {
            Logger::addToLog("RESPONSE ERROR: {$errno} - {$errmsg}");    
        }
        Logger::addToLog('');
        
        Logger::addToLog("RESPONSE INFO:");
        $lines = explode("\n", print_r($info, true));
        foreach ($lines as $line)
        {
            Logger::addToLog($line);
        ...
        Logger::addToLog("RESPONSE HEADERS:");
        $lines = explode("\n", $this->_response_headers);
        foreach ($lines as $line)
        {
            Logger::addToLog($line);
