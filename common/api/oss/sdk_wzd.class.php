...
//加载conf.inc.php文件
require_once OSS_API_PATH.DIRECTORY_SEPARATOR.'conf_wzd.inc.php';
...
class ALIOSS{
	 ...
	 * OSS服务地址
	 */
    const DEFAULT_OSS_HOST = OSS_HOST; #[Bucket概览] OSS外网域名
//    const DEFAULT_OSS_HOST = YII_ENV_PROD ? 'oss-cn-shanghai-internal.aliyuncs.com' : 'oss-cn-shanghai.aliyuncs.com'; #[Bucket概览] OSS外网域名
	...
	public function upload_file_by_file($bucket, $object, $file, $options = NULL){
		...
		/**
         * $extension之前是通过$file来获取
         * 但是由于实现的时候采用的是php中的tempname来做，后缀不能代表真实文件类型
         * 所以修改成通过$object目标文件来获取
         * change by yake
		 */
