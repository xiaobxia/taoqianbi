...
class Mns extends \yii\base\Component {
    ...
    public function topicGetCallbackMsg() {
        ...
        if(!isset($headers['Date'],$headers['X-Mns-Signing-Cert-Url'], $headers['Authorization'])) {
            throw new \Exception('回调数据未定义 Date X-Mns-Signing-Cert-Url Authorization', ErrCode::MISS_PARAMS);
        }
     ...
     */
    public function queueSendMessage($queue, $body) {
        $request = new SendMessageRequest($body);
