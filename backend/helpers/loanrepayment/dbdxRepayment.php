...
class dbdxRepayment extends abstractRepayment{
    ...
    public function getMoney(){
        $total_repay_amount = $this->repayment_amount + $this->repayment_amount * $this->loan_record_period->apr / 100 / 12 * $this->period;
        $loan_repayment_period_money = intval($total_repay_amount / $this->period);//每月应还款总额
        if(empty($loan_repayment_period_money)){
            throw new \Exception('每期还款总金额不能为空！');
        }
        return $loan_repayment_period_money;
    }
...
//        }
        ...
        //2.插入分期还款计划表
//        $result = $this->insertLoanRepaymentPeriod($total_repay_amount, $loan_repayment_id, $repay_date, $this->period, $this->loan_record_period, $period_amount);
        $result[0]['plan_will_repayment_amount'] = $total_repay_amount;
        for($i = 1; $i <= $this->period; $i++){
            ...
            $result[$i]['plan_repayment_principal'] = $this->repayment_amount / $this->period;
            $result[$i]['plan_repayment_interest'] = $period_amount - $result[$i]['plan_repayment_principal'];
            if($i == ($this->period)){
        ...
        if(!empty($fee_amount)){
            ...
            $loan_fee_repayment_period->period = 0;
            $loan_fee_repayment_period->plan_repayment_money = $fee_amount;
            $loan_fee_repayment_period->plan_will_repayment_amount = $total_repay_amount;
            $loan_fee_repayment_period->plan_repayment_time = $this->credit_repayment_time;
            $loan_fee_repayment_period->plan_next_repayment_time = strtotime($repay_date[0]);
            $loan_fee_repayment_period->status = LoanRepayment::STATUS_REPAYING;
            if(!$loan_fee_repayment_period->save()){
                throw new \Exception('抱歉，分期还款表服务费插入记录失败！');
            }
