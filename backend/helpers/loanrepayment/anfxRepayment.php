...
class anfxRepayment extends abstractRepayment{
    ...
    public function getMoney(){
        $anfx_repay = intval($this->repayment_amount * $this->loan_record_period->apr / 100 );//每年还款利息
        return $anfx_repay;
    }
    ...
    public function insertData(){
        ...
        $this->repay_count_num = intval($this->period / 12);
        ...
        if($operation == 1){
            ...
            $repayment_start_time = strtotime(date('Y-m-d', $this->repay_start_time) . '-12 month');
        }
        if(!empty($fee_amount)){
            ...
            $loan_fee_repayment_period->plan_will_repayment_amount = intval($this->repayment_amount + intval($this->repayment_amount * $this->loan_record_period->apr / 100 * $this->repay_count_num));
        ...
        //2.插入分期还款计划表
        $result[0]['plan_will_repayment_amount'] = intval($this->repayment_amount + intval($this->repayment_amount * $this->loan_record_period->apr / 100 * $this->repay_count_num));
        if($operation == 1){
            $next_repay_time = strtotime(date('Y-m-d', $this->repay_start_time) . '+12 month');
            $loan_fee_repayment_period = new LoanRepaymentPeriod();
            $loan_fee_repayment_period->loan_record_id = $this->loan_record_period->id;
            $loan_fee_repayment_period->repayment_id = $loan_repayment_id;
            $loan_fee_repayment_period->loan_person_id = $this->loan_record_period->loan_person_id;
            ...
            $result[0]['plan_will_repayment_amount'] = intval($this->repayment_amount - $period_amount + intval($this->repayment_amount * $this->loan_record_period->apr / 100 * $this->repay_count_num));
    ...
    public  function getPeriodRepayDate($sign_time = "", $period = ""){
        ...
        $time = strtotime(date('Y-m-01', $sign_time) . '+12 month');
        $repay_time = [];
        for($i = 1; $i <= $period; $i++){
            if ($days >= date('t', $time)) {
                $repay_date = date('Y-m-d', strtotime(date('Y-m-d', $time) . '+12 month -1 day'));
            } else {
                $repay_date =  date('Y-m-'.$days, $time);
            }
            array_push($repay_time, $repay_date);
            $time = strtotime(date('Y-m-01', $time) . '+12 month');
        }
        return $repay_time;
    }
}