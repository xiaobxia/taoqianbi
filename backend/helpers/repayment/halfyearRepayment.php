...
class halfyearRepayment extends abstractRepayment{
    private $total;//每半年还款总额

    public function getMoney(){
        $this->total = $this->platform_revenue / 100; //每半年还款总额
    }
    ...
    public function insertData(){
        ...
        foreach($result as $k => $v){
            $rows[] = [$this->financial_id, $v['date'], $v['done_principal'], $v['done_interest'], Financial::TYPE_REPAYMENT_HALF_YEAR,
                FinancialDetail::STATUS_VALID, FinancialDetail::ADD_TYPE_SYSTEM, Yii::$app->user->identity->username,$now, $now];
        }
