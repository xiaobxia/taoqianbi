...
class quarterRepayment extends abstractRepayment{

    private $total;//每季度还款总额

    public function getMoney(){
        $this->total = $this->platform_revenue / 100; //每季度还款总额
    }
    ...
    public function insertData(){
        ...
        for($i = 1; $i <= $peroid; $i++){
            ...
            $result[$i]['date'] = (date('Y-m-d',  strtotime('+ '.($i*3).' month', strtotime($this->sign_repayment_time))));
        }
        $result[$peroid]['done_principal'] = $this->total_amount_financing;
        $rows = [];
        foreach($result as $k => $v){
            $rows[] = [$this->financial_id, $v['date'], $v['done_principal'], $v['done_interest'], Financial::TYPE_REPAYMENT_QUARTER, FinancialDetail::STATUS_VALID, FinancialDetail::ADD_TYPE_SYSTEM, Yii::$app->user->identity->username,$now, $now];
        }
