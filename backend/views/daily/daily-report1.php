...
<input type="hidden" name="from_st" value="1">
...
<table class="tb tb2 fixpadding">
    ...
    <?php foreach ($daily_data as $value): ?>
        <tr class="hover">
            ...
            <td><?php  if(strtotime($value['date_time'])<1481731200){
                    echo 0;
                }else{
                    echo  empty($value['plan_repayment_total_money'])?"0%":sprintf("%0.2f",($value['overdue_s1_total_money']/$value['loan_total_money'])*100)."%";
                }  ?></td>
            <td>
                <?php
                if(strtotime($value['date_time'])<1481731200){
                    echo 0;
                }else{
                    echo empty($value['plan_repayment_total_money'])?"0%":sprintf("%0.2f",($value['overdue_s2_total_money']/$value['loan_total_money'])*100)."%";
                }
                ?>
            </td>
            <td>
                ...
                if(strtotime($value['date_time'])<1481731200){
                    ...
                    echo empty($value['plan_repayment_total_money'])?"0%":sprintf("%0.2f",($value['overdue_s3_total_money']/$value['loan_total_money'])*100)."%";
                }
                ?>
            </td>
            <td>
                ...
                if(strtotime($value['date_time'])<1481731200){
                    ...
                    echo empty($value['plan_repayment_total'])?"0%":sprintf("%0.2f",($value['overdue_s1_total']/$value['loan_total'])*100)."%";
                }
                ?>
            </td>
            <td>
                ...
                if(strtotime($value['date_time'])<1481731200){
                    ...
                    echo empty($value['plan_repayment_total'])?"0%":sprintf("%0.2f",($value['overdue_s2_total']/$value['loan_total'])*100)."%";
                }
                ?></td>
            <td><?php
                if(strtotime($value['date_time'])<1481731200){
                    ...
                    echo empty($value['plan_repayment_total'])?"0%":sprintf("%0.2f",($value['overdue_s3_total']/$value['loan_total'])*100)."%";
                }
                ?></td>
