       ...
       <tr class="hover">
           ...
           <?php elseif($value['fail_id']!=null):?>
               <th>
                   ...
                   <input type="hidden" name="num_id" value="<?php $num_id = $value['id']; echo $num_id;?>">
                   <input type="submit" name="search_submit" value="查看" class="btn">
                   <?php $form = ActiveForm::end(); ?>
               </th>
           <?php endif;?>
        ...
        <?php foreach ($data_list as $key=>$value): ?>
            <?php if (!empty($value)): ?>
                <tr>
                    ...
                    <?php if (!preg_match('/finish/',$value)): ?>
                        <th>
                            <?php $form = ActiveForm::begin(['action' => Url::toRoute('/'),'method' => "get",'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
                            <input type="hidden" name="r" value="solr-log/rewrite-insert">
                            <input type="hidden" name="num_id" value="<?php echo Yii::$app->getRequest()->get('num_id',''); ?>">
                            <input type="hidden" name="order_id" value="<?php $order_id = preg_replace('/\|[^\|]+/','',$value);if($order_id>100){echo $order_id;}?>">
                            <input type="submit" name="search_submit" value="重写" onclick="rewrite(<?php echo $order_id;?>)" class="btn">
                            <script type="text/javascript">
                                function rewrite($n_id){
                                    alert('将要重新写入的订单ID为：'+$n_id);
                                }
                            </script>
                            <?php $form = ActiveForm::end(); ?>
                        </th>
                    <?php elseif (preg_match('/finish/',$value)) : ?>
                        <th>
                            <input type="submit" name="search_submit" value="已重写" class="btn">
                        </th>
                    <?php endif; ?>
                </tr>
            <?php endif; ?>
...
<table>
    <tr>
        <th>
            <?php $form = ActiveForm::begin(['action' => Url::toRoute('/'),'method' => "get",'options' => ['style' => 'margin-top: 10px;margin-bottom:10px;'] ]); ?>
            <input type="hidden" name="r" value="solr-log/rewrite-insert-special">
            <input type="hidden" name="num_id" value="<?php echo Yii::$app->getRequest()->get('num_id',''); ?>">
