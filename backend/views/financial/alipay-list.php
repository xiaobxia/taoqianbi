...
$this->shownav('financial', 'menu_debit_alipay_list');
$this->showsubmenu('支付宝还款列表');
...
支付宝订单ID：<input type="text" value="<?php echo Yii::$app->getRequest()->get('alipay_order_id', ''); ?>" name="alipay_order_id" class="txt" style="width:280px;">&nbsp;
支付宝账号：<input type="text" value="<?php echo Yii::$app->getRequest()->get('alipay_account', ''); ?>" name="alipay_account" class="txt" style="width:120px;">&nbsp;
用户姓名：<input type="text" value="<?php echo Yii::$app->getRequest()->get('alipay_name', ''); ?>" name="alipay_name" class="txt" style="width:120px;">&nbsp;
最后操作人：<input type="text" value="<?php echo Yii::$app->getRequest()->get('operator_user', ''); ?>" name="operator_user" class="txt" style="width:120px;">&nbsp;
状态：<?php echo Html::dropDownList('status', Yii::$app->getRequest()->get('status', ''), AlipayRepaymentLog::$status, ['prompt' => '所有状态']); ?>&nbsp;
来源：<?php echo Html::dropDownList('source', Yii::$app->getRequest()->get('source', ''), AlipayRepaymentLog::$source_list, ['prompt' => '所有状态']); ?>&nbsp;
公司账号：<?php echo Html::dropDownList('type', Yii::$app->getRequest()->get('type', ''), AlipayRepaymentLog::$types, ['prompt' => '所有状态']); ?>&nbsp;
<br/>
转账时间：<input type="text" value="<?php echo Yii::$app->getRequest()->get('pay_at_begin', ''); ?>" name="pay_at_begin" onfocus="WdatePicker({startDate:'%y-%M-%d %H:%m:%s',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,readOnly:false})">
至 <input type="text" value="<?php echo Yii::$app->getRequest()->get('pay_at_end', ''); ?>"  name="pay_at_end" onfocus="WdatePicker({startDate:'%y-%M-%d %H:%m:%s',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,readOnly:false})">&nbsp;
...
至 <input type="text" value="<?php echo Yii::$app->getRequest()->get('updated_at_end', ''); ?>"  name="updated_at_end" onfocus="WdatePicker({startDate:'%y-%M-%d %H:%m:00',dateFmt:'yyyy-MM-dd HH:mm:00',alwaysUseStartDate:true,readOnly:true})">
    ...
    <table class="tb tb2 fixpadding" style="text-align: center;">
        ...
        <?php foreach ($info as $value): ?>
            <tr class="hover" style="text-align: center;">
                ...
                <td><?php echo isset(AlipayRepaymentLog::$types[$value->type]) ? AlipayRepaymentLog::$types[$value->type] : $value->type ; ?></td>
                <td><?php echo $value->alipay_order_id; ?></td>
                <td><?php echo $value->alipay_account; ?></td>
                <td><?php echo $value->alipay_name; ?></td>
                ...
                <td>
                <?php
                    echo isset(AlipayRepaymentLog::$status[$value->status]) ? AlipayRepaymentLog::$status[$value->status] : "---";
                ?>
                </td>
                <td><?php echo isset(AlipayRepaymentLog::$source_list[$value->source]) ? AlipayRepaymentLog::$source_list[$value->source] : '未知'; ?></td>
                <td><?php echo $value->alipay_date; ?></td>
                <td><?php echo date('Y-m-d H:i',$value->updated_at); ?></td>
                <td><?php echo $value->remark; ?></td>
                <td><?php echo $value->operator_user; ?></td>
                <td><?php echo $value->remark_admin; ?></td>
                <td >
                    <?php if (($value['status'] != AlipayRepaymentLog::STATUS_FINISH && $value['status'] != AlipayRepaymentLog::STATUS_BACK)): ?>
                        <a onclick="return confirmMsg('确认置为已处理')" href="<?php echo Url::toRoute(['financial/finish-alipay-log', 'id' => $value->id]); ?>">置为已处理</a>
                        <a onclick="return confirmMsg('确认置为退回')"  href="<?php echo Url::toRoute(['financial/finish-alipay-log', 'id' => $value->id,'type'=>1]); ?>">置为退回</a>
                    <?php else: ;?>
                        <a onclick="return confirmMsg('确认置为需人工')" href="<?php echo Url::toRoute(['financial/manual-alipay-log', 'id' => $value->id]); ?>">置为需人工</a>
                        <a onclick="return confirmMsg('确认置为未处理')" href="<?php echo Url::toRoute(['financial/reset-alipay-log', 'id' => $value->id]); ?>">置为未处理</a>
                    <?php endif; ?>
    ...
    function setRemarkTwo(id){
        ...
        if(remark_two == ""){
           ...
           $("#set_admin_remark input[name=remark_admin]").val(remark_two);
            $.ajax({
                url : "<?php echo Url::toRoute('financial/set-alipay-admin-remark')?>",
                type : 'json',
                method : 'post',
