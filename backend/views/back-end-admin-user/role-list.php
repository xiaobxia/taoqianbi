...
<table class="tb tb2 fixpadding">
	...
	<tr class="hover">
		...
		<td class="td23">
            <a href="<?php echo Url::toRoute(['back-end-admin-user/role-details','role'=> $value->name])?>">成员</a>
			<?php if ($value->name != AdminUser::SUPER_ROLE && ($value->groups != AdminUserRole::TYPE_COLLECTION) ): ?>
			<a href="<?php echo Url::toRoute(['back-end-admin-user/role-edit', 'id' => $value->id]); ?>">编辑</a>
                <a href="<?php echo Url::toRoute(['back-end-admin-user/menu-edit', 'id' => $value->id]); ?>">编辑菜单</a>
			<a onclick="return confirmMsg('确定要删除吗？\n删除后该角色对应的管理员将失去权限');" href="<?php echo Url::toRoute(['back-end-admin-user/role-delete', 'id' => $value->id]); ?>">删除</a>
			<?php else: ?>
            <?php endif; ?>
		</td>
	</tr>
