...
$this->showsubmenu('管理员管理', array(
	array('列表', Url::toRoute('back-end-admin-user/list'), 1),
	array('添加管理员', Url::toRoute('back-end-admin-user/add'), 0),
));
...
<table class="tb tb2 fixpadding">
	...
	<tr class="hover">
		...
		<td><?php echo $value->phone?$value->phone:'未输入'; ?></td>
		<td style="word-wrap: break-word; word-break: normal;word-break:break-all;max-width:260px;"><?php echo $value->role; ?></td>
		<td><?php echo $value->created_user; ?></td>
		<td><?php echo date('Y-m-d', $value->created_at); ?></td>
		<td><?php echo $value->mark; ?></td>
		<td class="td24">
			<a href="<?php echo Url::toRoute(['back-end-admin-user/change-pwd', 'id' => $value->id]); ?>">修改密码</a>
			<?php if ($value->username != AdminUser::SUPER_USERNAME): ?>
			<a href="<?php echo Url::toRoute(['back-end-admin-user/edit', 'id' => $value->id]); ?>">编辑</a>
			<a onclick="return confirmMsg('确定要删除吗？');" href="<?php echo Url::toRoute(['back-end-admin-user/delete', 'id' => $value->id]); ?>">删除</a>
			<?php endif; ?>
		</td>
	</tr>
	<?php endforeach; ?>
