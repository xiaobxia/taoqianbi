...
<table class="tb tb2 fixpadding">
    <tr>
        <th class="partition" colspan="15">
            <span style="font-size:20px"><?php echo $loanPerson['name'];?>的聚信立基本报告历史查询记录</span>
        </th>
        <th>请选择历史查询时间:
            <select  class="selectShow">
                <?php foreach ($creditJxl as $key=>$val): ?>
...
<?php if(empty($creditJxl)):?>
    暂无报告
<?php else:?>

    <?php
     $i=0;
     foreach($creditJxl as $val):
...
<div class="selectShow" id="tb_<?php echo $i++; ?>" >
 <?php if($type == 1):?>
    <table class="tb tb2 fixpadding">
        <tr>
            <td>
            <span style="color:red">数据更新时间：<?php echo empty($val['updated_at'])?'未获取报表':date('Y-m-d H:i:s',$val['updated_at']);?></span>
            </td>
        </tr>
        <tr>
    ...
    <table class="tb tb2 fixpadding">
        <tr>
            <th class="partition" colspan="25">
                <span style="font-size:20px">聚信立数据整理</span>
            </th>
        </tr>
