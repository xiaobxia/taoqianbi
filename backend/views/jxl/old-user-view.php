...
<?php if(empty($creditMg)):?>
  <table class="tb tb2 fixpadding">
    <tr>
        <th class="partition" colspan="15">
            <span style="font-size:20px"><?php echo $loanPerson['name'];?>的蜜罐信息历史查询记录</span>
        </th>
    <tr/>
    <tr>
        <th>无历史查询记录</th>
    ...
    <tr>
        ...
        <th>请选择历史查询时间:
            <select name="" class="selectShow">
                <?php foreach ($creditMg as $key=>$val): ?>
                    <option value="<?php echo $key ?>"><?php echo $val['update_time'] ?></option>
                <?php endforeach; ?>
            </select>
        </th>
    </tr>
