...
<table style="width: 650px;">
    ...
    <tr>
        ...
        <?php if($create_type == LoanProject::TYPE_STAGE_CONSUME):?>
            <td><?php echo $form->field($model, 'type')->dropDownList(LoanProject::$type_consume, ['prompt' => '请选择借款类型']); ?></td>
        <?php elseif($create_type == LoanProject::TYPE_TRUST): ?>
            <td><?php echo $form->field($model, 'type')->dropDownList(LoanProject::$type_trust, ['prompt' => '请选择借款类型']); ?></td>
    ...
    <tr>
        ...
        <td><?php echo $form->field($model, 'apply_time')->textInput(array("onfocus"=>"WdatePicker({startDate:'%y/%M/%d %H:%m:%s',dateFmt:'yyyy-MM-dd HH:mm:ss',alwaysUseStartDate:true,readOnly:false});")) ?></td>
    </tr>
    <tr>
...
<script>
    ...
    $("#loanrecordperiod-type").change(function(){
        ...
        $.ajax({
            type: 'get',
            url: '<?php echo Url::toRoute(['loan-period/switch-type']); ?>',
            async: false,//同步刷新
            data: {
                key: type
            },
    ...
    $("#loanrecordperiod-loan_project_id").change(function(){
        ...
        $.ajax({
            type: 'get',
            url: '<?php echo Url::toRoute(['loan-period/switch-shop']); ?>',
            async: false,//同步刷新
            data: {
                key: type
            },
