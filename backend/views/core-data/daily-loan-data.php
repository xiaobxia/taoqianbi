...
<?php $form = ActiveForm::begin(['id' => 'search_form','method'=>'get', 'action'=>Url::toRoute(['core-data/day-data-statistics','type'=>'loan_num','search_date'=>'2']), 'options' => ['style' => 'margin-top:5px;']]); ?>
...
<input type="submit" name="search_submit" value="过滤" class="btn">
    ...
    &nbsp;&nbsp;<input style="display: none" type="submit" name="submitcsv" value="导出csv" onclick="$(this).val('exportnum');return true;" class="btn">
    ...
    <form name="listform" method="post">
        <table class="tb tb2 fixpadding">
            ...
            <tr class="hover">
                ...
                <!-- 所有用户 -->
                <td class="td25"><?php echo isset($total_info['expire_num_0']) ? floor($rate*$total_info['expire_num_0']) : 0; ?></td>
                <td class="td25"><?php echo $total_info['repay_zc_num_0'] ?? 0; ?></a></td>
                <td class="td25"><?php echo isset($total_info['repay_num_0']) ? floor($rate*$total_info['repay_num_0']) : 0; ?></td>
                ...
                <!-- 新用户 -->
                <td class="td25" style="text-align:center;color:blue"><?php echo isset($total_info['expire_num_1'])?floor($rate*$total_info['expire_num_1']):0; ?></td>
                <td class="td25" style="text-align:center;color:blue"><?php echo empty($total_info['t_expire_num_1']) ? '0%' : sprintf("%0.2f",($total_info['t_expire_num_1']-$total_info['t_repay_zc_num_1'])/$total_info['t_expire_num_1']*100)."%"; ?></td>
                <td class="td25" style="text-align:center;color:blue"><?php echo empty($total_info['expire_num_1']) ? '-' : sprintf("%0.2f",($total_info['repay_num_1']/$total_info['expire_num_1'])*100)."%"; ?></td>
                <td class="td25" style="text-align:center;color:blue"><?php echo ((isset($total_info['t_expire_num_1']) && isset($total_info['t_repay_num_1'])) ? $total_info['t_expire_num_1']-$total_info['t_repay_num_1'] : '-'); ?></td>
                ...
                <!-- 老用户 -->
                <td class="td25" style="text-align:center;color:red"><?php echo isset($total_info['expire_num_2'])?floor($rate*$total_info['expire_num_2']):0; ?></td>
                <td class="td25" style="text-align:center;color:red"><?php echo empty($total_info['t_expire_num_2']) ? '0%' : sprintf("%0.2f",($total_info['t_expire_num_2']-$total_info['t_repay_zc_num_2'])/$total_info['t_expire_num_2']*100)."%"; ?></td>
                <td class="td25" style="text-align:center;color:red"><?php echo empty($total_info['expire_num_2']) ? '-' : sprintf("%0.2f",($total_info['repay_num_2']/$total_info['expire_num_2'])*100)."%"; ?></td>
                <td class="td25" style="text-align:center;color:red"><?php echo ((isset($total_info['t_expire_num_2']) && isset($total_info['t_repay_num_2'])) ? $total_info['t_expire_num_2']-$total_info['t_repay_num_2'] : '-'); ?></td>
            ...
            <?php foreach ($info as $key=> $value): ?>
                <tr class="hover" style="<?php echo date('w', $value['unix_time_key']) == 0 || date('w', $value['unix_time_key']) == 6 ?'background:#d1d0fb':'';?>">
                    <!-- 借款信息 -->
                    ...
                    <!-- 所有用户 -->
                    <td class="td25"><a href="<?php echo Url::toRoute(['pocket/pocket-list','plan_fee_time'=>$value['time_key'],'page_type'=>'3']); ?>" target="_blank"><?php echo isset($value['expire_num_0'])?floor($rate*$value['expire_num_0']):0; ?></a></td>
                    <td class="td25"><?php echo $value['repay_zc_num_0'] ?? 0; ?></a></td>
                    <td class="td25"><a href="<?php echo Url::toRoute(['pocket/pocket-list','plan_fee_time'=>$value['time_key'],'_status'=>4,'page_type'=>'3']); ?>" target="_blank"><?php echo isset($value['repay_num_0'])?floor($rate*$value['repay_num_0']):0; ?></a></td>
                    ...
                    <td class="td25"><a href="<?php echo Url::toRoute(['pocket/pocket-list','plan_fee_time'=>$value['time_key'],'overdue_day'=>1,'page_type'=>'3']); ?>" target="_blank"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : $value['expire_num_0']-$value['repay_num_0']; ?></a></td>
                    ...
                    <!-- 新用户 -->
                    <td class="td25" style="text-align:center;color:blue"><?php echo isset($value['expire_num_1'])?floor($rate*$value['expire_num_1']):0; ?></td>
                    <td class="td25" style="text-align:center;color:blue"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : (empty($value['expire_num_1'])?'0%':sprintf("%0.2f",($value['expire_num_1']-$value['repay_zc_num_1'])/$value['expire_num_1']*100)."%"); ?></td>
                    <td class="td25" style="text-align:center;color:blue"><?php echo empty($value['expire_num_1'])?'-':sprintf("%0.2f",($value['repay_num_1']/$value['expire_num_1'])*100)."%"; ?></td>
                    <td class="td25" style="text-align:center;color:blue"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : ((isset($value['expire_num_1']) && isset($value['repay_num_1'])) ? $value['expire_num_1']-$value['repay_num_1'] : '-'); ?></td>
                    ...
                    <!-- 老用户 -->
                    <td class="td25" style="text-align:center;color:red"><?php echo isset($value['expire_num_2'])?floor($rate*$value['expire_num_2']):0; ?></td>
                    <td class="td25" style="text-align:center;color:red"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : (empty($value['expire_num_2'])?'0%':sprintf("%0.2f",($value['expire_num_2']-$value['repay_zc_num_2'])/$value['expire_num_2']*100)."%"); ?></td>
                    <td class="td25" style="text-align:center;color:red"><?php echo empty($value['expire_num_2'])?'-':sprintf("%0.2f",($value['repay_num_2']/$value['expire_num_2'])*100)."%"; ?></td>
                    <td class="td25" style="text-align:center;color:red"><?php echo ($value['unix_time_key'] >= $now_date_time) ? '-' : ((isset($value['expire_num_2']) && isset($value['repay_num_2'])) ? $value['expire_num_2']-$value['repay_num_2'] : '-'); ?></td>
