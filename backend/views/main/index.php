...
/**
 * renderTopMenu(Array $topMenu)
 * 渲染顶部菜单
 * -----------------------------
 * @param Array $topMenu 顶部菜单
 * -----------------------------
 * @author Verdient。
 */
function renderTopMenu($topMenu){
    foreach($topMenu as $key => $value){
        $name = $value[0];
        $url = $value[1];
        echo '<li><em><a href="' . $url . '" id="header_'. $key .'" hidefocus="true" onmouseover="previewheader(\'' . $key . '\')" onmouseout="previewheader()" onclick="toggleMenu(\'' . $key . '\', \''.$url.'\');doane(event);">'. $name . '</a></em></li>';
    }
}

/**
 * renderLeftMenu(Array $leftMenu)
 * 渲染左侧菜单
 * -----------------------------
 * @param Array $leftMenu 左侧菜单
 * -----------------------------
 * @author Verdient。
 */
        ...
        if(is_array($value)){
            foreach($value as $menu) {
                if($menu[0] && $menu[1] == 'groupbegin'){
                    ...
                    $hide = false;
                    $element .= '<li class="s"><div class="lsub' . ($hide ? '' : ' desc').'" subid="' . $id . '"><div onclick="lsub(\'' . $id . '\', this.parentNode)">' . $menu[0] . '</div><ol style="display:' . ($hide ? 'none' : '') . '" id="' . $id . '">';
                }else if($menu[0] && $menu[1] == 'groupend') {
                    $element .= '<li class="sp"></li></ol></div></li>';
                }else if($menu[0] && !empty($menu[1])) {
                    $element .= '<li><a href="' . $menu[1] . '" hidefocus="true" target="main"><em onclick="menuNewwin(this)" title="新窗口打开"></em>' . $menu[0] . '</a></li>';
                }
            }
        }
        echo '<ul id="menu_' . $key . '" style="display: none">' . $element . '</ul>';
    }
}

$basescript = '';
$extra = isset($_GET['action']) ? ('r=' . urlencode($_GET['action'])) : Url::toRoute('main/home');
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>后台管理系统</title>
        <link rel="stylesheet" href="<?php echo $BASEURL;?>/image/admincp.css?v=2015072101" type="text/css" media="all" />
        <link rel="stylesheet" href="<?php echo $BASEURL;?>/css/site.css" type="text/css" media="all" />
        <link rel="shortcut icon" href="<?php echo $BASEURL;?>/favicon.ico">
        <script src="<?php echo $BASEURL;?>/js/dcz_common.js" type="text/javascript"></script>
    </head>
    <body style="margin: 0px" scroll="no">
        ...
        <table id="frametable" cellpadding="0" cellspacing="0" width="100%" height="100%">
            <tr>
                <td colspan="2" height="90">
                    <div class="mainhd">
                        <a href="#" class="logo"><?php echo APP_NAMES;?></a>
                        <div class="uinfo" id="frameuinfo">
                            <p>您好，<em><?php echo $phone;?></em> [<a href="<?php echo $logoutUrl;?>" target="_top">退出</a>][<a href="<?php echo $resetRoleUrl;?>" target="_top">刷新权限</a>]</p>
                            <p class="btnlink"><a href="#">站点首页</a></p>
                        </div>
                        <div class="navbg"></div>
                        <div class="nav">
                            <ul id="topmenu">
                                <?php renderTopMenu($topMenu);?>
                            </ul>
            ...
            <tr>
                <td valign="top" width="160" class="menutd">
                    <div id="leftmenu" class="menu">
                        <?php renderLeftMenu($leftMenu);?>

                    </div>
                </td>
                <td valign="top" width="100%" class="mask">
                    <iframe src="<?php echo $extra ?>" id="main" name="main" width="100%" height="100%" frameborder="0" scrolling="yes" style="overflow: visible;display:"></iframe>
                </td>
            </tr>
        </table>
        <div id="scrolllink" style="display: none">
            <span onclick="menuScroll(1)"><img src="<?php echo $BASEURL; ?>/image/scrollu.gif" /></span><span onclick="menuScroll(2)"><img src="<?php echo $BASEURL; ?>/image/scrolld.gif" /></span>
        </div>
        ...
        <script type="text/JavaScript">
            ...
            var cookiedomain = '';
            var cookiepath = '';
            var headers = new Array('<?php echo $headers ?>');
            var admincpfilename = '<?php echo $basescript ?>';
            var menukey = '';
            var headerST = null;
