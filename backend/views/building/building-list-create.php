...
$this->shownav('building', 'menu_building-list-create');
$this->showsubmenu('借款创建');
?>
...
借款人编号：<input type="text" value="<?php echo Yii::$app->getRequest()->get('id_number', ''); ?>" name="id_number" class="txt" style="width:120px;">&nbsp;
借款人性质：<input type="text" value="<?php echo Yii::$app->getRequest()->get('property', ''); ?>" name="property" class="txt" style="width:120px;">&nbsp;
    ...
    <table class="tb tb2 fixpadding">
        ...
        <?php foreach ($loan_person as $value): ?>
            <tr class="hover">
                ...
                <th><?php echo $value['credit_limit']; ?></th>
                <th><?php if(empty($value['attachment'])): ?>
                        待上传
                    <?php else: ?>
                        <a target="_blank" href="<?php echo Url::toRoute(['loan/loan-person-pic', 'id' => $value['id']])?>">查看</a>
                    <?php endif;?>
                </th>
                <th><?php echo date("Y-m-d H:i",$value['created_at']); ?></th>
                <td>
                    <a href="<?php echo Url::toRoute(['building/building-backend-add', 'person_id' => $value['id'], 'user_id' => $value['uid']]); ?>">借款</a>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
