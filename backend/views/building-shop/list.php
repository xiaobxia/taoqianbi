 ...
 */
$this->shownav('business', 'menu_shop_list');
$this->showsubmenu('商户列表');
?>
<?php $form = ActiveForm::begin(['id' => 'searchform','method'=>'get', 'options' => ['style' => 'margin-bottom:5px;']]); ?>
所属项目：<?php echo Html::dropDownList('project', Yii::$app->getRequest()->get('project', ''), $projects, array('prompt' => '-所有商户-')); ?>&nbsp;
商户状态：<?php echo Html::dropDownList('status', Yii::$app->getRequest()->get('status', ''), Shop::$shop_status, array('prompt' => '-所有状态-')); ?>&nbsp;
...
<table class="tb tb2 fixpadding">
    ...
    <tr class="hover">
        ...
        <td>
            <?php $loan_project = LoanProject::find()->where(['id'=>$v['loan_project_id']])->one();?>
