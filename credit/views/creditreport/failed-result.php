...
<div id="pay_result_wraper">
    <div class="column">
            <p class="logo_wraper a_center"><img src="<?= Yii::$app->request->getAbsoluteBaseUrl(); ?>/image/installment-shop/layouts/logo.png" width="10%"></p>
    ...
    <div class="btn_wraper m_center">
        <div style="background:<?= $color;?> none repeat scroll 0 0" class="_btn bg_fd5353 em_1_2 fff a_center _b_radius" onclick="jumpTo('<?= !empty($url) ? $url : Url::toRoute(['credit-app/index']);?>')">返回首页</div>
    </div>
...
$(document).ready(function () {
    ...
    // 进入页面打点
    setWebViewFlag();
    MobclickAgent.onEvent('zhima_fault');
});
function jumpTo(url) {
    if (typeof nativeMethod == 'undefined') {
        ...
        nativeMethod.returnNativeMethod('{"type":"7"}');
    }
}
</script>