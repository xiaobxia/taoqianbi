 ...
 * @param url 分享url
 * @param logo 分享logo
 * @param type 0|1，0直接分享，1右上角出现分享按钮 2 | 单独分享
 * @param callback   ""，回调函数名字
 * @param platform QQ,QZONE,WEIXIN,WEIXIN_CIRCLE
 * @param extend   "ooo=uu&"
 * @param is_upload ""
 * @returns 
 */
function nativeShare(title, body, url, logo, type, platform,extend,callback,is_upload) {
    var type = type > 0 ? type : 0;
    var platform = platform ? platform : "QQ,QZONE,WEIXIN,WEIXIN_CIRCLE,SMS,SMS_INVITE";
    var extend   = extend ? extend : "" ;
    var callback   = callback ?  callback : "";
    var is_upload  = is_upload ? 0 : 1;
    var json_str = '{"share_title":"' + title + '","share_body":"' + body + '","share_url":"' + url + '","share_logo":"' + logo + '","type":"' + type + '","platform":"' + platform + '","extend":"' + extend + '","callback":"' + callback + '","is_upload":"' + is_upload + '"}';
    console.log(json_str);
    return nativeMethod.shareMethod(json_str);
}
