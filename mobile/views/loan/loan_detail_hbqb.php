...
<div class="wrap repayment-detail">
        ...
        <ul>
            <?php foreach ($list as $value) { ?>
                <li class="<?= isset($value['class']) ? $value['class'] : '' ?>">
                    <h1 style="position:relative;"><?= $value['title'] ?><?php if(isset($value['btn_url']) && $value['btn_url']){ ?>
                    <a style="position:absolute;right:0;display:inline-block;padding:0.072rem 0.2rem;color:#fff;font-size:0.3466666667rem;text-decoration:none;border-radius:8px;" href="<?= ApiUrl::toNewh5(['discover/more'])?>">获取更多机会</a><?php } ?></h1><p><?= $value['body'] ?></p>
                </li>
            <?php } ?>
        </ul>
    </div>
    ...
    <div class="item">
        <ul>
            <li><span>借款金额</span><i><?= sprintf("%0.2f", $order['money_amount'] / 100) ?>元</i></li>
            <li><span>实际到账</span><i><?= sprintf("%0.2f", $order['true_money_amount'] / 100) ?>元</i></li>
            <li><span>综合费用</span><i><?= sprintf("%0.2f", $order['counter_fee'] / 100) ?>元</i></li>
            <?php if ($repayment) { ?>
                <?php if (!empty($repayment['coupon_money'])) { ?>
                    <li><span>抵扣金额</span><i><?= sprintf("%0.2f", $repayment['coupon_money'] / 100) ?>元</i></li>
                <?php } ?>
            <?php } ?>
            ...
            <?php if($flag_show == 1){ ?>
                <li>
                	<span>协议说明</span>
                	<a href="<?php echo $url_one;?>">《借款协议》</a>
                	<a href="<?php echo $url_two;?>">《平台服务协议》</a><br>
                	<a class="last" href="<?php echo $url_three;?>">《授权扣款委托书》</a>
                </li>
            <?php }?>
        </ul>
    </div>
    <?php if ($repayment) { ?>
        <div class="item">
            <ul>
                <li>
                    <span>待还金额</span>
                    <i><?= sprintf("%0.2f", $repayment['remain_money_amount'] / 100) ?>元</i>
                </li>
                <li>
                    <span>已还金额</span>
                    <i><?= sprintf("%0.2f", $repayment['true_total_money'] / 100) ?>元</i>
                </li>
                <li>
                    <span>最迟还款日期</span>
        ...
        <?php if ($repayment['status'] != UserLoanOrderRepayment::STATUS_REPAY_COMPLETE) { ?>
            <?php if ($remainDelayTimes > 0 && !$repayment['is_overdue']) { ?>
                    ...
                    <?php } ?>
                    <a class="fixed-button" href="<?= Url::toRoute(['loan/loan-repayment-type', 'id' => $order['id']]) ?>">立即还款</a>
            <?php } else { ?>
                <a class="fixed-button" href="<?= Url::toRoute(['loan/loan-repayment-type', 'id' => $order['id']]) ?>">立即还款</a>
            <?php } ?>
        <?php } ?>
    ...
    */ 
    function getShowType(){
        ...
        switch (showType){
            ...
            case 1:
                $(".repayment-detail.wrap .content ul").find("li").eq(0).addClass("sType0"); //待审核1
                break;
            case 2: 
                $(".repayment-detail.wrap .content ul").find("li").eq(1).addClass("sType1"); //打款中2
                break;
            case 5:
                $(".repayment-detail.wrap .content ul").find("li").eq(2).addClass("sType2"); //待还款5
                break;
            case 4:
                $(".repayment-detail.wrap .content ul").find("li").eq(3).addClass("sType3"); // 已逾期4
                break;
            case 6:
                $(".repayment-detail.wrap .content ul").find("li").eq(1).addClass("sType3"); // 审核未通过6
                break;
            case 3:
                $(".repayment-detail.wrap .content ul").find("li").eq(3).addClass("sType4");
                break;
                // 打款成功3
        }
