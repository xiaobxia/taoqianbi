...
abstract class NetUtil
{
	 ...
	 * @param		int				$exe_timeout, 执行超时时间, 默认为3s
	 *
	 * @return		sting/bool		$data, 为返回数据, 失败返回 false
	 */
	public static function cURLHTTPGet($url, $timeout = 2, $host = '', $exe_timeout = 3) {
		self::clearError();
		...
		// 执行超时
		curl_setopt($curl_handle, CURLOPT_TIMEOUT, $exe_timeout);
		// HTTP返回错误时, 函数直接返回错误
	 ...
	 * @return		sting/bool		$data, 为返回数据, 失败返回 false
	 */
	public static function cURLHTTPPost($url, $post_data, $timeout = 2, $host = '', $header_append = array(), $exe_timeout = 3) {
		self::clearError();
